	...
	*/
		...
		return function(url,fun,err,dom,callBackFunName){
			...
			if(url.match(/\.css$/)){
				...
				_.x(url,function(H){
					...
					/* 替换模板内样式地址 */
					H=H.replace(/<link rel="stylesheet" href="https://www.hostker.com/static/js/.+?">/igm,function(o){
						return o.replace(/<link rel="stylesheet" href="https://www.hostker.com/static/js/(.+?)">/i,function(all,url){
							if(!url.match(/^http/))
	...
	_S.enHtml=function(){
		return this.replace(/(^\s*)|(\s*$)/g,'')
			...
			.replace(/(^|[^\"\'\]>])(http|ftp|mms|rstp|news|https|telnet)\:\/\/([\w\/.#&!?%:;=\-_]+)/g,'$1<a href="https://www.hostker.com/static/js/$2://$3" rel="external nofollow noreferer" class="link" target="_blank">$2://$3</a>')
			.replace(/\n/g,"<br>");
	};
